...
//
// Regression fixtures modeled on real workbook calculations.
...
// A nested-IIF "ladder" mapping long program names to categories (special chars: - / &).
const PROGRAM =
`IIF([Program Name]="North Region Retail - Online/Store Strategy","North Region Retail",
IIF([Program Name]="South Region Wholesale & Distribution - Partner Strategy","South Region Wholesale",
IIF([Program Name]="East Region Field Services - Franchise Strategy","East Region Services",
"Other")))`;
...
// The office-initials pattern from the first bug report.
const OFFICE_IN =
`IIF([Sales Office]="Springfield North Office","SN",
IIF([Sales Office]="Riverside South Office","RS",
""))`;
...
describe('real-world Tableau calculations', () => {
  ...
  describe('nested IIF ladders parse into a nested symbol tree', () => {
    it('Program nests beyond depth 1 and captures every IIF + field reference', () => {
      const tree = treeOf(PROGRAM);
      ...
      expect(all.filter(s => s.type === SymbolType.FieldReference && s.name === 'Program Name').length).toBeGreaterThanOrEqual(3);
    });
    ...
    it('OFFICE_IN captures both branches', () => {
      const all = flatten(treeOf(OFFICE_IN));
