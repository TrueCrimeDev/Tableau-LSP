function tS(h){h.classList.toggle('c');const b=h.nextElementSibling;if(b&&b.classList.contains('sb'))b.classList.toggle('c')}
        function tSS(h){h.classList.toggle('c');const b=h.nextElementSibling;if(b&&b.classList.contains('ssb'))b.style.display=b.style.display==='none'?'':'none'}
        function openSection(id){const h=document.getElementById(id);if(h&&!h.classList.contains('c'))return;if(h&&h.classList.contains('c'))tS(h);}
        ...
        document.querySelectorAll('.sh').forEach(function(el) {
            el.addEventListener('click', function(event) {
                if (event.target.closest('.ha')) { return; }
                tS(el);
            });
        });
        document.querySelectorAll('.ssh').forEach(function(el) {
            el.addEventListener('click', function() { tSS(el); });
        });
        ...
        window.addEventListener('error', function(ev) {
            ...
            if (el) { el.textContent = '[JS ERROR] ' + (ev.message || String(ev.error)) + ' (' + ev.filename + ':' + ev.lineno + ')'; el.style.display = ''; }
        });
        window.addEventListener('unhandledrejection', function(ev) {
            var el = document.getElementById('workbook-empty-state');
            if (el) { el.textContent = '[UNHANDLED PROMISE] ' + String(ev.reason); el.style.display = ''; }
        });
        ...
        if (requiredElements.some(element => !element)) {
            ...
            themeList.addEventListener('click', event => {
                ...
                if (!theme) { setStatus('No theme available.', 'error'); return; }
                const colors = normalizeColorList(theme.colors);
                if (colors.length === 0) { setStatus('Theme has no colors.', 'error'); return; }
                const action = button.dataset.action;
                if (action === 'load-theme') {
                    const palette = { name: ensureUniqueName(theme.name), type: normalizePaletteType(theme.type), colors };
                    upsertPalette(palette);
                    state.selectedName = palette.name;
                    state.editor = { name: palette.name, type: palette.type, colors: palette.colors.slice() };
                    renderAll();
                    ...
                    vscode.postMessage({ type: 'applyToWorkbook', palette: { name: theme.name, type: normalizePaletteType(theme.type), colors } });
        ...
        window.addEventListener('message', event => {
            ...
            vscode.postMessage({ type: 'webviewDiag', message: '[v6] handler fired, type=' + message.type });
            try {
                ...
                if (message.type === 'workbookParsed') {
                    ...
                    vscode.postMessage({ type: 'webviewDiag', message: '[v6] workbookParsed branch hit, fileName=' + message.fileName });
                    const _emptyDbg = document.getElementById('workbook-empty-state');
                    if (_emptyDbg) { _emptyDbg.textContent = '[v6] workbookParsed received — rendering\u2026'; _emptyDbg.style.display = ''; }
                    try {
                        renderWorkbookData(message);
                        vscode.postMessage({ type: 'webviewDiag', message: '[v6] renderWorkbookData completed without throw' });
                    } catch (renderErr) {
                        console.error('[WBI v6] renderWorkbookData threw:', renderErr);
                        vscode.postMessage({ type: 'webviewDiag', message: '[v6] renderWorkbookData THREW: ' + String(renderErr) });
                        const _eEl = document.getElementById('workbook-empty-state');
                        if (_eEl) { _eEl.textContent = 'Render error: ' + String(renderErr); _eEl.style.display = ''; }
                    }
                }
                ...
                if (message.type === 'workbookError') {
                    ...
                    if (emptyEl) {
                        emptyEl.textContent = typeof message.message === 'string'
                            ? message.message
                            : 'Workbook error.';
                    }
                }
                if (message.type === 'extractResult') {
                    ...
                    if (extractBtn) {
                        ...
                        extractBtn.innerHTML = '<svg class="ic"><use href="#i-check"/></svg> Extracted ' + count + ' calculation' + (count !== 1 ? 's' : '');
                        extractBtn.classList.remove('bp'); extractBtn.classList.add('bs');
                        extractBtn.style.borderLeft = '3px solid #3794ff';
                        setTimeout(() => {
                            extractBtn.innerHTML = orig;
                            extractBtn.classList.remove('bs'); extractBtn.classList.add('bp');
                ...
                vscode.postMessage({ type: 'webviewDiag', message: '[v6] OUTER handler threw: ' + String(handlerErr) });
            }
        });
        ...
        function renderPaletteList() {
            ...
            if (badge) { badge.textContent = String(state.palettes.length); }
            ...
            paletteList.innerHTML = state.palettes.map((palette, index) => {
                ...
                return '<div class="ri' + selClass + '" data-index="' + index + '">' +
                    '<div class="cb" style="background:' + gradient + '"></div>' +
                    '<span class="lb">' + name + '</span>' +
                    '<span class="mt">' + colors.length + '</span>' +
                    '<div class="ra">' +
                      '<button class="ib" data-action="edit" data-index="' + index + '" title="Edit"><svg class="ic"><use href="#i-pen"/></svg></button>' +
                      '<button class="ib" data-action="apply" data-index="' + index + '" title="Apply"><svg class="ic"><use href="#i-arrow"/></svg></button>' +
                      '<button class="ib" data-action="archive" data-index="' + index + '" title="Archive"><svg class="ic"><use href="#i-dots"/></svg></button>' +
                    '</div>' +
                '</div>';
            }).join('');
        }
        ...
        function renderColors() {
            ...
            const chips = state.editor.colors.map((color, index) => {
                ...
                return '<div class="cp" data-index="' + index + '" style="background:' + safeColor + '" title="' + escapeHtml(safeColor) + '"></div>';
            });
        ...
        function renderThemes() {
            ...
            themeList.innerHTML = themePresets.map((theme, index) => {
                ...
                return '<div class="ri">' +
                    ...
                    '<span class="mt">' + colors.length + '</span>' +
                    '<div class="ra">' +
                      '<button class="ib" data-action="load-theme" data-index="' + index + '" title="Load"><svg class="ic"><use href="#i-load"/></svg></button>' +
                      '<button class="ib" data-action="apply-theme" data-index="' + index + '" title="Apply"><svg class="ic"><use href="#i-arrow"/></svg></button>' +
                    '</div>' +
                '</div>';
            }).join('');
        }
        ...
        function renderScalePreview() {
            ...
            scalePreview.innerHTML = state.scaleColors.map(color => {
                ...
                return '<div class="sw" style="background:' + safeColor + '"><span class="tp">' + safeColor + '</span></div>';
            }).join('');
        }
        ...
        function highlightFormula(formula) {
            ...
            result = result.replace(/\b(IF|THEN|ELSE|ELSEIF|END|CASE|WHEN|AND|OR|NOT|TRUE|FALSE|NULL)\b/gi,
        ...
        function renderWorkbookData(data) {
            ...
            if (!data) {
                if (fileCard) { fileCard.style.display = 'none'; fileCard.innerHTML = ''; }
                if (emptyState) { emptyState.textContent = 'Open a\u202f.twb or .twbx file to inspect its contents.'; emptyState.style.display = ''; }
                if (extractWrap) { extractWrap.style.display = 'none'; }
                ['wb-datasources-content','wb-calcs-content','wb-fields-content','wb-sheets-content','wb-palettes-content'].forEach(id => {
                ...
                if (workbookSbEl2) {
                    workbookSbEl2.querySelectorAll('.ssh:not(.c)').forEach(function(h) { tSS(h); });
                }
                return;
            }

            if (emptyState) { emptyState.style.display = 'none'; }
            if (fileCard) { fileCard.style.display = ''; renderFileCard(data); }
            ...
            if (workbookSbEl2) {
                workbookSbEl2.querySelectorAll('.ssh.c').forEach(function(h) { tSS(h); });
            }
        }
        ...
        function renderFileCard(data) {
            ...
            const versionPart = data.tableauVersion ? 'Tableau\u00A0' + escapeHtml(data.tableauVersion) + '\u00A0\u00B7\u00A0' : '';
            const ds = data.datasourceCount || 0;
            const cl = data.calcCount || 0;
            const sh = data.sheetCount || 0;
            const meta = versionPart + ds + '\u00A0datasource' + (ds !== 1 ? 's' : '') + '\u00A0\u00B7\u00A0' + cl + '\u00A0calc' + (cl !== 1 ? 's' : '') + '\u00A0\u00B7\u00A0' + sh + '\u00A0sheet' + (sh !== 1 ? 's' : '');
            const safePath = escapeHtml(data.filePath || '');
            el.innerHTML = '<svg class="ic"><use href="#i-twb"/></svg>' +
                '<div class="wb-file-info">' +
                  '<div class="wb-file-name" title="' + safePath + '">' + fileName + '</div>' +
                  '<div class="wb-file-meta">' + meta + '</div>' +
                '</div>' +
            ...
            if (revealBtn && data.filePath) {
                ...
                revealBtn.addEventListener('click', () => { vscode.postMessage({ type: 'revealFile', path: fp }); });
            }
        }
        ...
        function renderWorksheets(worksheets) {
            ...
            el.innerHTML = worksheets.map(name => {
                return '<div class="tree-item">' +
        ...
        function renderWorkbookPaletteList(palettes) {
            ...
            if (!el) { return; }
            if (!palettes.length) {
                el.innerHTML = '<div class="sub-empty">No custom palettes in this workbook.</div>';
                return;
            }
            el.innerHTML = palettes.map((palette, index) => {
                const colors = normalizeColorList(palette.colors);
                const gradient = buildGradient(colors);
                const name = escapeHtml(palette.name);
                return '<div class="ri">' +
                    ...
                    '<span class="mt">' + colors.length + '</span>' +
                    '<div class="ra">' +
                      '<button class="ib" data-action="import-workbook" data-index="' + index + '" title="Import to library"><svg class="ic"><use href="#i-import"/></svg></button>' +
                    '</div>' +
                '</div>';
            }).join('');
        }
