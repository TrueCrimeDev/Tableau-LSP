...
const vscode = {
  ...
  workspace: {
    ...
    openTextDocument: async (path) => {
      ...
      return {
        uri: typeof path === 'string' ? vscode.Uri.file(path) : path,
    ...
    executeCommand: async (command, ...args) => {
      ...
      if (command === 'vscode.executeHoverProvider') {
        ...
        return [{
          ...
          range: new vscode.Range(position.line, position.character, position.line, position.character + 3)
        }];
      }
      ...
      if (command === 'vscode.executeDocumentSymbolProvider') {
        return [
          { name: 'SUM', kind: 'function', range: new vscode.Range(7, 0, 7, 10) },
          { name: 'AVG', kind: 'function', range: new vscode.Range(8, 0, 8, 10) },
          { name: 'COUNT', kind: 'function', range: new vscode.Range(9, 0, 9, 10) },
          { name: 'CASE', kind: 'keyword', range: new vscode.Range(25, 0, 29, 3) }
        ];
      }
...
async function main() {
  try {
    console.log('Starting Tableau LSP tests...');
