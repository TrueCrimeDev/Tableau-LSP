...
export function parseDocument(document: TextDocument): {
    ...
    for (const { m, type } of allMatches) {
        ...
        if (type === SymbolType.Keyword) {
            ...
            } else if (keywordName === 'ELSEIF' || keywordName === 'ELSE' || keywordName === 'WHEN') {
                // These keywords start a new branch, so they should be children of the main block
                blockStack.pop();
                parent = blockStack[blockStack.length - 1];
                parent.children!.push(symbol);
                blockStack.push(symbol);
            } else if (keywordName === 'END') {
                // Pop the current branch (e.g. the ELSE)
                blockStack.pop();
                // Now the parent is the main block (IF/CASE)
                parent = blockStack[blockStack.length - 1];
                parent.end = symbol;
                // Pop the main block
                blockStack.pop();
