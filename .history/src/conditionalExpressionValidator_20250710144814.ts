...
export class ConditionalExpressionValidator {
    ...
    private validateIfBranchLogic(branches: Symbol[], parentSymbol: Symbol): void {
        ...
        for (const branch of branches) {
            if (branch.name === 'THEN') {
                hasThen = true;
                break; // Found THEN, that's what we need
            }
        }
        
        // Check for IF without THEN - this is a critical error
        // Only flag if we have an IF with conditions but no THEN
        if (!hasThen && parentSymbol.text && parentSymbol.text.trim()) {
            this.addDiagnostic(
                parentSymbol.range,
