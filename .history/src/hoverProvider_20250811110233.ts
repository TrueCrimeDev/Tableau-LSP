...
function detectCalculationContext(document: TextDocument, position: { line: number; character: number }): CalculationContext | null {
    try {
        ...
        for (let line = position.line; line >= 0; line--) {
            ...
            if (!text.trim()) continue; // skip blank lines while searching upwards
            const match = text.match(headerRegex);
            if (match) {
                const name = match[1];
                const description = match[2];
                // Determine body range starting at header line
                const body = determineBodyRange(document, line);
                if (body) {
                    return { headerLine: line, name, description, bodyStart: body.start, bodyEnd: body.end };
                }
                return null;
            }
            // Stop if we hit a non-comment, non-blank line before finding a header
