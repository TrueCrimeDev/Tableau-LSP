...
import {
	a_Vars, action, twblUris, twblVars, tableauVersion, alpha_11, alpha_3, builtinCommands_v1,
	commentTags, configCache, diagnostic, hoverCache, inactiveVars, isTwbl2_h, lexers, libDirs,
	...
	// ahkVersion, ahkVars, ahkUris, // Commented out as they are AutoHotkey specific
} from './common';
...
export interface CallSite extends TableauSymbol {
	offset?: number
	paraminfo?: ParamInfo
}
...
export interface TableauSymbol extends DocumentSymbol {
	alias?: string
	alias_range?: Range
	module?: string
	cached_types?: Array<string | TableauSymbol>
	children?: TableauSymbol[]
	data?: unknown
	decl?: boolean
	def?: boolean
	...
	ignore?: boolean
	overwrite?: number
	parent?: TableauSymbol
	returns?: number[] | null
	since?: string
	static?: boolean | null
	type_annotations?: Array<string | TableauSymbol> | false
	type_name?: string
	uri?: string
}

export interface FuncNode extends TableauSymbol {
	assume: FuncScope
	closure?: boolean
	params: Variable[]
	...
	hasref: boolean
	variadic: boolean
	labels: Record<string, TableauSymbol[]>
	declaration: Record<string, TableauSymbol>
...
export interface ClassNode extends TableauSymbol {
	base?: TableauSymbol
	full: string
	extends: string
	extendsuri?: string
	...
	generic_types?: (string | TableauSymbol)[][]
	type_params?: Record<string, TableauSymbol>
}

export interface Variable extends TableauSymbol {
	arr?: boolean | 2			// *, *,  ...
...
export interface Property extends Variable {
	local?: Record<string, TableauSymbol>
	declaration?: Record<string, TableauSymbol>
	unresolved_vars?: Record<string, Variable>
	has_this_param?: boolean
	params?: Variable[]
...
namespace FuncNode {
	export function create(name: string, kind: SymbolKind, range: Range, selectionRange: Range, params: ParamList, children?: TableauSymbol[], isstatic?: boolean | null): FuncNode {
...
const S2O: Record<string, TableauSymbol> = {
	$DIRPATH,
	$DLLFUNC,
	$FILEPATH,
	ANY,
...
export class Lexer {
	...
	public children: TableauSymbol[] = [];
	public d = 0;
	public d_uri = '';
	public declaration: Record<string, TableauSymbol> = {};
	...
	public labels: Record<string, TableauSymbol[]> = {};
	...
	public typedef: Record<string, TableauSymbol> = {};
	...
	constructor(document: TextDocument, scriptdir?: string, d = 0) {
		...
		let continuation_sections_mode: boolean | null, currsymbol: TableauSymbol | undefined;
		...
		let allow_$ = true, block_mode = true, format_mode = false, h = isTwbl2_h;
		...
		if (d || /\.d\.(twbl?|twb)$/i.test(this.fsPath)) {
			...
			this.parseScript = function (): void {
				...
				if (this.d < 0)
					return;
				if (this.d & 2) {
					const overwrite = uri.endsWith('/twbl_h.d.twbl') ? 1 : 0;
					let t;
					for (const [k, it] of Object.entries(this.declaration)) {
						switch (it.kind) {
							...
							case SymbolKind.Class:
								...
								if (!(t = twblVars[k]) || overwrite >= (t.overwrite ?? 0))
									twblVars[k] = it;
								break;
							case SymbolKind.Variable:
								if (it.def)
									twblVars[k] = it, it.uri = uri;
								break;
						}
					}
				}
				...
				function parse_types(sym: TableauSymbol) {
					const types = parse();
					if (!(lk = tokens[--j]))
					...
					return j;
					function parse(): Array<string | TableauSymbol> {
						let t: Token, has_typeof: boolean, r: string, tp;
						const tps: (string | TableauSymbol)[] = [];
							...
							switch (lk.type) {
								case TokenType.Identifier:
									...
									if (lk?.content === '<' && !lk.topofline) {
										const generic_types: (string | TableauSymbol)[][] = [];
										while (true) {
								...
								case TokenType.BlockStart: {
									let full = '';
									const props: Record<string, TableauSymbol> = {}, b = lk;
			...
			const d_path = this.fsPath.replace(/\.\w+$/, '.d.twbl');
			...
			this.parseScript = function (): void {
				...
				parser_pos = 0, last_LF = -1, customblocks = { region: [], bracket: [] }, continuation_sections_mode = false, h = isTwbl2_h;
				...
				if (!process.env['BROWSER']) {
					// const m = this.d_uri && find_d_twbl(resolve_scriptdir(this.d_uri)) || find_d_twbl(d_path); // Commented out due to missing find_d_twbl
					// if (m) // Commented out due to missing find_d_twbl
					// 	includetable[this.d_uri = m.uri] = m.path; // Commented out due to missing find_d_twbl
					// else this.d_uri = ''; // Commented out due to missing find_d_twbl
				}
				parse_unresolved_typedef();
		...
		function parse_unresolved_typedef() {
			for (const m of Object.values(comments)) {
				if (m.data !== null || !/^[ \t]*\*?[ \t]*@typedef\s/m.test(m.content))
					continue;
				parse_jsdoc_detail(_this, m.content, {} as TableauSymbol);
			}
		}
		...
		function stop_parse(tk: Token, allow_skip = false, message = diagnostic.maybev1()) {
			...
			// switch (_this.actionwhenv1 ??= configCache.ActionWhenV1IsDetected) { // Commented out due to missing ActionWhenV1IsDetected
			switch (_this.actionwhenv1 ??= 'Continue') { // Default to 'Continue'
				case 'SkipLine': {
					if (!allow_skip)
						return true;
		...
		function parse_block(mode = BlockType.Script, _parent = _this as unknown as TableauSymbol, classfullname: string = ''): TableauSymbol[] {
			const result: TableauSymbol[] = [], { document, line_ranges, tokens } = _this;
			let tk = tokens[parser_pos - 1] ?? EMPTY_TOKEN, lk = tk.previous_token ?? EMPTY_TOKEN;
			let blocks = 0, next = true, _low = '';
			let _cm: Token | undefined, tn: TableauSymbol | undefined;
			...
			function parse_line(end?: string, act?: string, min = 0, max = 1, pi?: ParamInfo): TableauSymbol[] {
				let b: number, hascomma = 0, t = 0, nk: Token | undefined, e: number;
				const res: TableauSymbol[] = [], info = pi ?? { offset: 0, count: 0, comma: [], miss: [], unknown: false };
			...
			function parse_expression(rpair = ',', end?: string): TableauSymbol[] {
			...
			function parse_obj(must = false): boolean {
				...
				const mark: Token[] = [], props: Record<string, TableauSymbol> = {};
			...
			function addvariable(token: Token, md: number = 0, p?: TableauSymbol[]) {
			...
			function adddeclaration(node: FuncNode | ClassNode) {
				...
				if (node.kind === SymbolKind.Class) {
					...
					children.forEach((it: TableauSymbol) => {
		...
		function set_extends(tn: ClassNode, str: string) {
			...
			if (!tn.extendsuri && tn.extends.startsWith('#'))
					// tn.extendsuri = ahkVars[(tn.extends = tn.extends.substring(1)).toUpperCase()]?.uri ?? ahkUris.ahk2; // Commented out due to missing ahkVars/ahkUris
					tn.extendsuri = ''; // Placeholder
		}
				...
				if (!m.trim()) {
					...
					if (isdll) {
						if (existsSync(m) && statSync(m).isDirectory())
							...
							if (!/\.\w+$/.test(m))
								m = m + '.dll';
							// m = findLibrary(m, [], dlldir || _this.scriptpath, true)?.path ?? m; // Commented out due to missing findLibrary
							if (m.includes(':'))
						...
						if (tk) {
							if (m.startsWith('*')) {
								...
								return;
							} else if (!(m = findLibrary(m, _this.libdirs, includedir)) || !existsSync(m.path)) { // Commented out due to missing findLibrary
								if (!ignore)
							...
							if (mode & BlockType.Mask) _this.addDiagnostic(diagnostic.unsupportinclude(), tk.offset, tk.length,
								{ code: DiagnosticCode.include, severity: DiagnosticSeverity.Warning });
						} else if ((m = find_d_ahk(m))) // Commented out due to missing find_d_ahk
		...
		// function find_d_ahk(path: string) { // Commented out due to missing findLibrary
		// 	if (path.startsWith('<'))
		...
		function set_detail(sym: TableauSymbol, cm: Token) {
		...
		interface JsDoc {
			...
			vars?: {
				[name: string]: {
					detail: string,
					type_annotations?: (string | TableauSymbol)[] | false,
					type_str?: string
				}
			}
		}

		function parse_jsdoc_detail(lex: Lexer, detail: string, sym: TableauSymbol): JsDoc {
			sym.detail = detail;
			...
			function remove_types(tps: (string | TableauSymbol)[], remove: TableauSymbol[], add?: ClassNode) {
				for (let i = tps.length - 1; i >= 0; i--)
					remove.includes(tps[i] as TableauSymbol) && tps.splice(i, 1);
				add && tps.push(add);
				return add;
			}
			function add_obj_type(sym: TableauSymbol, name: string): ClassNode {
	...
	public findSymbol(name: string, kind?: SymbolKind, position?: Position)
		: {
			node: TableauSymbol, uri: string, is_global?: boolean | 1	// maybe
			is_this?: boolean, parent?: TableauSymbol, scope?: TableauSymbol
		} | null | undefined {
		let node: TableauSymbol | undefined, scope: FuncNode | undefined, uri = this.uri;
		...
		let t: TableauSymbol | undefined, parent: TableauSymbol | undefined, is_global: boolean | 1 = true;
		if (name.startsWith('$'))
	...
	public getContext(position: Position, ignoreright = false): Context {
		...
		let kind: SymbolKind, symbol: TableauSymbol | undefined, token: Token | undefined, start: number, is_end_expr, text;
	...
	public searchScopedNode(pos: Position, root?: TableauSymbol[]): TableauSymbol | undefined {
		const { line, character } = pos;
		let its: TableauSymbol[] | undefined, cls: ClassNode, fn: FuncNode, offset: number;
	...
	public getScopeSymbols(scope?: TableauSymbol): Record<string, Variable> {
	...
	private addSymbolFolding(symbol: TableauSymbol, first_brace: number) {
...
function createPrototype(name: string, kind = 0, extends_ = '') {
	return { name, full: name, kind, extends: extends_, range: ZERO_RANGE, selectionRange: ZERO_RANGE, uri: '' } as TableauSymbol;
}
...
export function getClassBase(node: TableauSymbol, lex?: Lexer) {
	...
	// cls = findClass(lex ?? lexers[ahkUris.ahk2], name, uri)!; // Commented out due to missing ahkUris
	cls = findClass(lex ?? lexers[''], name, uri)!; // Placeholder
	return iscls ? cls : cls?.prototype;
}

export function getClassMember(lex: Lexer, node: TableauSymbol, name: string, ismethod: boolean | null, bases?: (ClassNode | null)[]): TableauSymbol | undefined {
...
export function getClassMembers(lex: Lexer, node: TableauSymbol, bases?: ClassNode[]): Record<string, TableauSymbol> {
...
function getClassOwnProps(doc: Lexer, node: TableauSymbol) {
...
export function decltypeExpr(lex: Lexer, tk: Token, end_pos: number | Position, _this?: ClassNode): TableauSymbol[] {
	...
	while ((tk = op_stack.pop()!))
		calculate(tk);
	const result = new Set<TableauSymbol>;
	let syms: Set<TableauSymbol> | TableauSymbol[] = [], that;
	for (tk of stack) {
		if (tk.symbol) {
...
export function decltypeInvoke(lex: Lexer, syms: Set<TableauSymbol> | TableauSymbol[], name: string, call: boolean, paraminfo?: ParamInfo, _this?: ClassNode) {
	const tps = new Set<TableauSymbol>;
	let that = _this;
	for (let n of syms) {
...
function decltypeByref(sym: Variable, lex: Lexer, types: TableauSymbol[], _this?: ClassNode) {
	...
	function resolve(it: TableauSymbol, prop: string, types: TableauSymbol[], needthis = 0) {
		switch (it.kind) {
			case SymbolKind.Method:
				needthis++;
			// fall through
			...
			case SymbolKind.Property: {
				...
				for (const t of annotations) {
					if (t === VARREF)
						return true;
					if ((t as TableauSymbol).data !== VARREF)
						continue;
			...
			case SymbolKind.Class: {
				...
				if (iscall) {
					if (n.kind === SymbolKind.Class)
						...
						let tps: TableauSymbol[] | Set<TableauSymbol> = decltypeReturns(n, lexers[n.uri!] ?? lex, cls);
...
function decltypeVar(sym: Variable, lex: Lexer, pos: Position, scope?: TableauSymbol, _this?: ClassNode): TableauSymbol[] {
	...
	let ts: TableauSymbol[] | undefined, t;
	for (const sym of syms) {
		...
		if (sym.is_param && sym.pass_by_ref) {
			const tt = new Set<TableauSymbol>;
			for (const t of ts) {
				if (t === VARREF)
					return [ANY];
				if (t.data === VARREF)
	...
	if (sym.for_index !== undefined) {
		...
		function resolve(it: TableauSymbol, invoke_enum = true) {
...
function decltypeTypeAnnotation(annotations: (string | TableauSymbol)[], lex: Lexer, _this?: ClassNode, type_params?: Record<string, TableauSymbol>) {
	const types = new Set<string | TableauSymbol>;
	let is_typeof;
...
function resolveCachedTypes(tps: (string | TableauSymbol)[], resolved_types: Set<TableauSymbol>, lex: Lexer,
	_this?: ClassNode, type_params?: Record<string, TableauSymbol>) {
	...
	function resolve_generic_type(cls: ClassNode): TableauSymbol {
...
export function decltypeReturns(sym: TableauSymbol, lex: Lexer, _this?: ClassNode): TableauSymbol[] {
	let types: Set<TableauSymbol> | undefined, ct: Array<string | TableauSymbol> | undefined, is_typeof, has_obj;
		...
		case true: {
			...
			for (let tp of annotations) {
				...
				if ('\'"'.includes(tp[0]))
					...
					types.add(FLOAT);
				else types.add(`${is_typeof ? 'typeof ' : ''}${tp}` as unknown as TableauSymbol);
			}
			if (types.has(ANY))
...
export function typeNaming(sym: TableauSymbol) {
	let s;
	switch (sym.kind) {
...
export function generateTypeAnnotation(sym: TableauSymbol, lex?: Lexer, _this?: ClassNode) {
	return joinTypes((sym.type_annotations || decltypeReturns(sym, lexers[sym.uri!] ?? lex, _this)));
}

export function joinTypes(tps?: Array<string | TableauSymbol> | false) {
	if (!tps) return '';
...
function resolveTypeAnnotation(annotation?: string) {
	if (annotation) {
		const lex = new Lexer(TextDocument.create('', 'twbl', 0, `$:${annotation}`), undefined, -1);
		lex.parseScript();
...
export function findSymbol(lex: Lexer, fullname: string, kind?: SymbolKind, pos?: Position) {
	...
	if (!res)
		return;
	let p = name.length, parent: TableauSymbol | undefined, node: typeof parent = res.node;
	node.uri ??= res.uri;
	for (name of names) {
		switch (fullname[p]) {
			default: return;
	...
	function find_builtin_symbol(name: string) {
		if ((t = twblVars[name]))
			return t;
		for (const uri of [twblUris.twbl_h, twblUris.twbl])
			if ((t = lexers[uri]?.typedef[name]))
				return t.uri ??= uri, t;
		if (notdef && !l)
			if ((t = lexers[uri = twblUris.winapi]?.declaration[name]))
				return t.uri ??= uri, t;
	}
...
export function getSymbolDetail(sym: TableauSymbol, lex: Lexer, remove_re?: RegExp): string | MarkupContent {
...
export function makeDupError(a: TableauSymbol, b: TableauSymbol): string {
	if (a.kind === b.kind) {
...
export function checkDupError(decs: Record<string, TableauSymbol>, syms: TableauSymbol[], lex: Lexer) {
	const { children, diagnostics, uri, relevance } = lex;
	const severity = DiagnosticSeverity.Error;
	let l = '', v1: Variable, v2: Variable;
	// if (ahkVersion < alpha_11) // Commented out due to missing ahkVersion
	// 	Object.assign(relevance, { [ahkUris.ahk2]: 1, [ahkUris.ahk2_h ?? '\0']: 1 }); // Commented out due to missing ahkUris
	for (const it of syms) {
		...
		switch ((v1 = it as Variable).kind) {
			...
			case SymbolKind.Module:
				...
				// if (l.substring(0, 2) === 'A_' && ahkVars[l]) { // Commented out due to missing ahkVars
				if (l.substring(0, 2) === 'A_') { // Modified to remove ahkVars dependency
					if (is_var) continue;
...
export function isContinuousLine(lk: Token, tk: Token, parent?: TableauSymbol): boolean {
	switch (lk.type) {
		case TokenType.EOF:
		case TokenType.Comma:
		case TokenType.Assign:
