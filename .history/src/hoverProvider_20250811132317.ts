...
function createHoverForWordAtPosition(document: TextDocument, position: any, fieldParser: FieldParser | null): Hover | undefined {
    ...
    if (fieldMatch && fieldParser) {
        ...
        // Return undefined field hover only when the hovered word is part of the field token
        if (wordRange && fieldName.toUpperCase().includes(document.getText(wordRange).toUpperCase())) {
...
function createUndefinedFieldHoverResponse(fieldName: string, range: any): Hover | undefined {
    const parts: string[] = [];
    // Consistent styling: title then explanation
    parts.push('Undefined Field');
    parts.push(`[${fieldName}] is not defined in the current context.`);
