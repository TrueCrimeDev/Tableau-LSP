...
import { ActionType, TableauSymbol, FormatOptions, Lexer, ZERO_RANGE, fixupFormatOptions, traverseInclude } from './lexer';
...
// export * from './colorProvider'; // File does not exist
// export * from './commandProvider'; // File does not exist
export * from './completionProvider';
export * from './constants';
export * from './definitionProvider';
// export * from './formattingProvider'; // File does not exist
export * from './tableauHoverProvider'; // Corrected file name
export * from './lexer';
...
export { URI, TableauSymbol as AhkSymbol, MessageActionItem };
...
export interface LSConfig {
	...
	// InterpreterPath: string // Removed as it's AutoHotkey specific
	...
	WorkingDirs: string[]
	// ActionWhenV1IsDetected: ActionType; // Removed as it's AutoHotkey specific
}

export interface Utils {
	getTableauVersion?(paths: string[]): Promise<string[]>
	// getDllExport?(paths: string[] | Set<string>, onlyone?: boolean): Promise<string[]> // Removed as it's AutoHotkey specific
	// getRCData?(path?: string): { uri: string, path: string, paths?: string[] } | undefined // Removed as it's AutoHotkey specific
	sendTableauRequest?<T>(method: string, params: unknown[]): Promise<T | undefined>
	sendDiagnostics?(uri: string, diagnostics: Diagnostic[]): Promise<void>
	sendNotification?(method: string, params?: unknown): void
	sendRequest?<T>(method: string, params?: unknown): Promise<T>
	// setInterpreter?(path: string): Promise<void> // Removed as it's AutoHotkey specific
...
export const VERSION_STAGE: Record<string, number | string> = { ALPHA: 3, BETA: 2, RC: 1, 3: 'alpha', 2: 'beta', 1: 'rc' };
...
export const configCache: LSConfig = {
	...
	WorkingDirs: [],
	// ActionWhenV1IsDetected: 'Continue', // Removed
};
...
export let locale = 'en-us', rootDir = '', isTwbl2_h = false;
export let tableauPath = '', tableauPath_resolved = '';
export let tableauVersion = Infinity, reservedIndex = 0;
export let twblUris: Record<string, string> = {};
export let twblVars: Record<string, TableauSymbol> = {};
...
// Removed AutoHotkey-specific variables/objects and their exports
// export const a_Vars: Record<string, string> = {};
// export const ahkPath = '';
// export const ahkPath_resolved = '';
// export const ahkVersion = '';
// export const builtinVars: string[] = [];
// export const builtinVars_h: string[] = [];
// export let isahk2_h = false;
// export function setAhkPath(path: string) { /* implementation */ }
// export function setIsAhkH(v: boolean) { /* implementation */ }

// Dummy exports for now to resolve compilation errors in other files that might still import them
export const a_Vars: Record<string, string> = {};
export const ahkPath = '';
export const ahkPath_resolved = '';
export const ahkVersion = 0; // Use a number for version
export const builtinVars: string[] = [];
export const builtinVars_h: string[] = [];
export let isahk2_h = false;
export function setAhkPath(path: string) { /* no-op */ }
export function setIsAhkH(v: boolean) { /* no-op */ }
export const ahkVars: Record<string, any> = {}; // Dummy for ahkVars
export const ahkUris: Record<string, string> = {}; // Dummy for ahkUris

interface LibSymbol extends Array<TableauSymbol> {
	fsPath: string
	islib: boolean
}
...
export function openFile(path: string, showError = true): TextDocument | undefined {
	...
	if (text !== undefined)
		return TextDocument.create(URI.file(path).toString(), 'twbl', -10, text);
}
...
export function initCaches() {
	...
	twblVars = {}, twblUris = {}, inactiveVars = {};
	completionItemCache = {
		constant: [],
		directive: {
			'#': [],
		...
		snippet: [],
		text: [],
		option: { twbl_criteria: [], hotstring: [] }
	};
}

export function loadSyntax(filename = 'twbl', d = 3) {
	let path: string | undefined;
	const file = `${rootDir}/syntaxes/<>/${filename}`, files: string[] = [];
	if (process.env['BROWSER']) {
		getWorkspaceFile(`${file}.d.twbl`).then(v => v && load_td(TextDocument.create(v.uri, 'twbl', -10, v.text)));
		filename === 'twbl' && files.push(`${rootDir}/syntaxes/twbl_common.json`);
		...
		const td = (path = getfilepath('.d.twbl')) && openFile(restorePath(path));
		td && load_td(td);
		if (filename === 'twbl') {
			existsSync(path = `${rootDir}/syntaxes/twbl_common.json`) && files.push(path);
	...
	function load_td(td: TextDocument) {
		...
		lex.parseScript(), lexers[lex.uri] = lex, twblUris[filename] = lex.uri;
	}
			...
			switch (key) {
					...
					for (snip of arr) {
						...
						if (c === '#')
							hoverCache[snip.prefix.toLowerCase()] = [snip.prefix, {
								contents: { kind: 'markdown', value: '```twbl\n' + (snip.syntax ?? trim(snip.body)) + '\n```\n\n' + (snip.description ?? '') }
							}];
					}
					break;
				case 'keys':
				...
				case 'keywords':
					...
					for (snip of arr) {
						...
						hoverCache[snip.prefix.toLowerCase()] = [snip.prefix, { contents: { kind: 'markdown', value: '```twbl\n' + (snip.syntax ?? trim(snip.body)) + '\n```\n\n' + (snip.description ?? '') } }];
					}
					break;
				case 'snippet':
				...
				case 'variables':
					for (snip of arr) {
						twblVars[snip.body.toUpperCase()] = {
							name: snip.body,
...
export function enumFiles(dirpath: string, filter = /\.(twbl?|twb)$/i) {
...
export function versionMatch(requires: string) {
	...
	for (const req of requires.split('||')) {
		for (const m of req.matchAll(/(twbl_h\s*)?([<>]=?|=)?([^<>=]+)/g)) {
			if (m[1] && !isTwbl2_h) continue next;
			const v = versionEncode(m[3]);
			let result = false;
			switch (m[2] ?? '>=') {
				case '>=': result = tableauVersion >= v; break;
				case '<=': result = tableauVersion <= v; break;
				case '=': result = tableauVersion === v; break;
				case '>': result = tableauVersion > v; break;
				case '<': result = tableauVersion < v; break;
...
export function setIsTwblH(v: boolean) { isTwbl2_h = v; }
export function setTableauPath(path: string) {
	const resolved = resolvePath(path, true);
	if (resolved)
		tableauPath = path, tableauPath_resolved = resolved;
}
...
export function setVersion(version: string) {
	tableauVersion = versionEncode(version);
	reservedIndex = tableauVersion < alpha_11 ? 1 : 0;
}
