...
export class ConditionalExpressionValidator {
    ...
    private validateIfBranchLogic(branches: Symbol[], parentSymbol: Symbol): void {
        let hasThen = false;
        let hasElseOrElseIf = false;

        for (const branch of branches) {
            if (branch.name === 'THEN') {
                hasThen = true;
            }
            if (branch.name === 'ELSE' || branch.name === 'ELSEIF') {
                hasElseOrElseIf = true;
            }
        }

        // Only flag as error if there are ELSEIF/ELSE branches but no THEN
        // Do not flag single-branch IFs (e.g., IF ... END) or valid single-line IFs
        if (!hasThen && hasElseOrElseIf) {
            this.addDiagnostic(
                parentSymbol.range,
                `IF statement missing THEN clause (ELSEIF/ELSE present but no THEN)`,
                DiagnosticSeverity.Error,
