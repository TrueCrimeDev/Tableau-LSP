        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            if (this.isTableauExpression(line)) {
                ...
                const expressionItem = this.validationController.createTestItem(
                    `${uri.toString()}_expr_${lineIndex}`,
                    `Expression ${expressionCount}: ${line.substring(0, 50)}${line.length > 50 ? '...' : ''}`,
                    uri
                );
    ...
    private async runValidation(request: vscode.TestRunRequest, token: vscode.CancellationToken) {
        ...
        for (const testItem of this.getTestItems(request)) {
            ...
            try {
                ...
                const validation = await this.validateExpression(document, testItem, tableauProvider);
                ...
                run.failed(testItem, new vscode.TestMessage(`Validation error: ${error}`));
            }
        }
