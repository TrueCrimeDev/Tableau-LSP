...
async function registerAdditionalComponents(context: ExtensionContext): Promise<void> {
    try {
        ...
        const extractFromFileCmd = commands.registerCommand('tableau.extractFromFile', async (uri?: Uri) => {
            window.showInformationMessage('Extraction (beta) temporarily disabled in this prerelease build.');
        });

        const extractFromFolderCmd = commands.registerCommand('tableau.extractFromFolder', async () => {
            window.showInformationMessage('Folder extraction (beta) temporarily disabled in this prerelease build.');
        });
