...
function createJSDocHoverResponse(jsDocSymbol: JSDocSymbol, range: any): Hover {
    ...
    parts.push(`<h3><span style="color:var(--vscode-symbolIcon-functionForeground);">${escapeHtml(jsDocSymbol.name)}</span></h3>`);

    if (jsDocSymbol.deprecated) {
        parts.push(`<p><span style="color:var(--vscode-errorForeground);text-decoration:line-through;">⚠️ deprecated</span></p>`);
    }

    if (jsDocSymbol.since) {
        parts.push(`<p><em style="color:var(--vscode-descriptionForeground);">Since: ${jsDocSymbol.since}</em></p>`);
    }
    ...
    if (jsDocSymbol.markdown_detail) {
        parts.push(`<p style="color:var(--vscode-foreground);">${escapeHtml(jsDocSymbol.markdown_detail)}</p>`);
    } else if (jsDocSymbol.description) {
        parts.push(`<p style="color:var(--vscode-foreground);">${escapeHtml(jsDocSymbol.description)}</p>`);
    }

    // Add parameters section with structured layout
    if (jsDocSymbol.parameters && jsDocSymbol.parameters.length > 0) {
        ...
        jsDocSymbol.parameters.forEach(param => {
            const optional = param.optional ? '<span style="color:var(--vscode-charts-orange);">?</span>' : '';
            const defaultVal = param.defaultValue ? ` <span style="color:var(--vscode-charts-blue);">= ${escapeHtml(param.defaultValue)}</span>` : '';
            parts.push(`<li><code style="background:var(--vscode-textCodeBlock-background);padding:2px 4px;border-radius:3px;">${escapeHtml(param.name)}${optional}: <span style="color:var(--vscode-symbolIcon-typeForeground);">${escapeHtml(param.type)}</span>${defaultVal}</code> - ${escapeHtml(param.description)}</li>`);
        });
        parts.push('</ul>');
    }

    // Add returns section with type highlighting
    if (jsDocSymbol.returns) {
        parts.push('<hr>');
        const escapedReturns = jsDocSymbol.returns.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    ...
    // Add example section with proper code formatting
    if (jsDocSymbol.example) {
        parts.push('<hr>');
        parts.push(`<h4 style="color:var(--vscode-charts-green);">Example</h4>`);
        
        // Check if it contains multiple lines or function calls
        const hasNewlines = jsDocSymbol.example.includes('\n');
        const hasMultipleCalls = jsDocSymbol.example.includes(';') || jsDocSymbol.example.match(/\w+\(/g)?.length > 1;
        
        if (hasNewlines || hasMultipleCalls) {
            // Multi-line or complex example - use pre block
            const escapedExample = jsDocSymbol.example
                .replace(/&/g, '&amp;')
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;');
            ...
            if (jsDocSymbol.example.includes('(') && jsDocSymbol.example.includes(')')) {
                // Function call - use inline code with syntax highlighting
                const escapedExample = jsDocSymbol.example
                    .replace(/&/g, '&amp;')
                    .replace(/</g, '&lt;')
                    .replace(/>/g, '&gt;');
