...
export interface TableauSymbol {
    ...
    returnType?: string;
    category?: string;
    syntax?: string; // For control flow structures like IF, CASE
}
...
export class TableauProvider implements 
    ...
    /** Parse control flow structures like IF and CASE */
    private parseControlFlowStructure(lines: string[], startIndex: number, description: string): { symbol: TableauSymbol; nextIndex: number } | null {
        const line = lines[startIndex].trim();
        
        // Check for IF structure
        const ifMatch = /^IF\s+(.+):\s*(\w+)\s+THEN$/i.exec(line);
        if (ifMatch) {
            const [, testExpression, testType] = ifMatch;
            let i = startIndex + 1;
            let syntaxParts = [`IF ${testExpression}: ${testType} THEN`];
            
            // Parse the multi-line IF structure
            while (i < lines.length) {
                ...
                if (currentLine.startsWith('ELSEIF')) {
                    const elseifMatch = /^ELSEIF\s+(.+):\s*(\w+)\s+THEN$/i.exec(currentLine);
                    if (elseifMatch) {
                        const [, elseTestExpr, elseTestType] = elseifMatch;
                        syntaxParts.push(`ELSEIF ${elseTestExpr}: ${elseTestType} THEN`);
                        i++;
                        continue;
                    }
                } else if (currentLine.startsWith('ELSE')) {
                    syntaxParts.push('ELSE');
                    i++;
                    continue;
                } else if (currentLine === 'END') {
                    syntaxParts.push('END');
                    i++;
                    break;
                } else if (currentLine.match(/^\s*\w+.*:\s*\w+$/)) {
                    // This looks like a value line (e.g., "value_if_true: Any")
                    syntaxParts.push(`  ${currentLine}`);
                    i++;
                    continue;
                }
                
                // If we hit something that doesn't match the IF structure, break
                break;
            }
            
            // Build the syntax string from the parsed parts
            const syntaxString = syntaxParts.join('\n');
            
            // Create the IF symbol with proper syntax
            const symbol: TableauSymbol = {
                name: 'IF',
                type: 'keyword',
                description: description || 'Tests a series of expressions and returns a value for the first one that is TRUE.',
                category: 'Control Flow',
                syntax: syntaxString
            };
            
            return { symbol, nextIndex: i };
        }
        
        // Check for CASE structure
        const caseMatch = /^CASE\s+(.+):\s*(\w+)$/i.exec(line);
        if (caseMatch) {
            const [, expression, expressionType] = caseMatch;
            let i = startIndex + 1;
            let syntaxParts = [`CASE ${expression}: ${expressionType}`];
            
            // Parse the multi-line CASE structure
            while (i < lines.length) {
                const currentLine = lines[i].trim();
                if (!currentLine) {
                    i++;
                ...
                if (currentLine.startsWith('WHEN')) {
                    const whenMatch = /^WHEN\s+(.+):\s*(\w+)\s+THEN\s+(.+):\s*(\w+)$/i.exec(currentLine);
                    if (whenMatch) {
                        const [, value, valueType, returnVal, returnType] = whenMatch;
                        syntaxParts.push(`WHEN ${value}: ${valueType} THEN ${returnVal}: ${returnType}`);
                        i++;
                        continue;
                    }
                } else if (currentLine.startsWith('ELSE')) {
                    const elseMatch = /^ELSE\s+(.+):\s*(\w+)$/i.exec(currentLine);
                    if (elseMatch) {
                        const [, defaultVal, defaultType] = elseMatch;
                        syntaxParts.push(`ELSE ${defaultVal}: ${defaultType}`);
                        i++;
                        continue;
                    }
                } else if (currentLine === 'END') {
                ...
                // If we hit something that doesn't match the CASE structure, break
                break;
            }
            ...
            // Create the CASE symbol
            const symbol: TableauSymbol = {
                name: 'CASE',
                type: 'keyword',
                description: description || 'Goes through alternatives and returns a value for the first matching case.',
                category: 'Control Flow',
                syntax: syntaxString
            };
