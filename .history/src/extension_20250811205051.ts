import { ExtensionContext, commands, window, languages, workspace, Uri, ProgressLocation } from 'vscode';
...
async function registerAdditionalComponents(context: ExtensionContext): Promise<void> {
    try {
        ...
        const extractFromFileCmd = commands.registerCommand('tableau.extractFromFile', async (uri?: Uri) => {
            const target = uri || window.activeTextEditor?.document.uri;
            if (!target) {
                window.showWarningMessage('No file selected to extract from.');
                return;
            }
            await runExtraction([target]);
        });

        const extractFromFolderCmd = commands.registerCommand('tableau.extractFromFolder', async () => {
            const folder = await window.showOpenDialog({ canSelectFolders: true, canSelectFiles: false, canSelectMany: false });
            if (!folder || folder.length === 0) return;
            const uris: Uri[] = [];
            const walker = async (dir: Uri) => {
                const entries = await workspace.fs.readDirectory(dir);
                for (const [name, type] of entries) {
                    const child = Uri.joinPath(dir, name);
                    if (type === 2) { // File
                        if (name.toLowerCase().endsWith('.twb') || name.toLowerCase().endsWith('.twbx')) {
                            uris.push(child);
                        }
                    } else if (type === 1) {
                        await walker(child);
            ...
            await walker(folder[0]);
            if (uris.length === 0) {
                window.showInformationMessage('No .twb/.twbx files found.');
                return;
            }
            await runExtraction(uris);
        });

        const convertNotesCmd = commands.registerCommand('tableau.convertNotesToMarkdown', async () => {
            window.showInformationMessage('Markdown conversion stub (implement emit logic).');
        });

        const viewCalcsCmd = commands.registerCommand('tableau.viewCalculations', async () => {
            window.showInformationMessage('Calculations viewer stub (Tree/Webview to be implemented).');
        });

        context.subscriptions.push(extractFromFileCmd, extractFromFolderCmd, convertNotesCmd, viewCalcsCmd);
...
async function runExtraction(files: Uri[]) {
    const modZip = await import('./extract/zip.js');
    const results: any[] = [];
    await window.withProgress({ location: ProgressLocation.Notification, title: 'Extracting Tableau calculations', cancellable: false }, async () => {
        for (const f of files) {
            const calcs = await modZip.extractFromFile(f);
            results.push(...calcs);
        }
    });
    if (results.length === 0) {
        window.showInformationMessage('No calculations extracted.');
        return;
    }
    const notesLines: string[] = [];
    notesLines.push(`# Extracted Calculations (${results.length})`);
    for (const c of results) {
        notesLines.push(`\n## ${c.title}`);
        notesLines.push('```tableau');
        notesLines.push(`// !${c.title}`);
        notesLines.push(c.formula.trim());
        notesLines.push('```');
    }
    const doc = await workspace.openTextDocument({ content: notesLines.join('\n'), language: 'markdown' });
