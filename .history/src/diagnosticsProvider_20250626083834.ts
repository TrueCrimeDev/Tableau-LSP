...
import { ParsedDocument, FUNCTION_SIGNATURES, TableauSymbolType } from './common';

export function getDiagnostics(document: TextDocument, parsedDocument: ParsedTableauDocument): Diagnostic[] {
    ...
    for (const symbol of parsedDocument.symbols) {
        if (symbol.type === TableauSymbolType.FunctionCall) {
            const functionName = symbol.name.toUpperCase();
            const signature = UNCTION_SIGNATURES[functionName];
            if (signature) {
